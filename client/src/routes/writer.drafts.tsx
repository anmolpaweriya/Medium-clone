...
import { currentUser, draftsByAuthor } from "@/lib/mock-data";
          ...
          {drafts.map((d) => (
            <div key={d.id} className="flex items-center gap-4 rounded-lg border border-border/60 bg-card p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">Last edited {new Date(d.publishedAt).toLocaleDateString()} · {d.readMinutes} min read</div>
              </div>
              <Button asChild variant="outline" size="sm" className="rounded-full"><Link to="/writer/editor">Continue writing</Link></Button>
